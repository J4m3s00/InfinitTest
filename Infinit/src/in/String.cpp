...
namespace in {
	...
	int FindStringPosition(const String& string, const String& search, INUint offset)
	{
	...
	String GetBlock(const char* str, const char** outPosition)
	{
		...
		if (outPosition)
			*outPosition = end;
		INUint length = end - str + 1;
		return String(str, length);
	}

	String GetBlock(const String& string, INUint offset)
	{
		const char* str = string.c_str() + offset;
		return GetBlock(str);
	}
	...
	int NextInt(const String& string)
	{
		...
		for (INUint i = 0; i < string.size(); i++)
		{
			if (isdigit(string[i]))
